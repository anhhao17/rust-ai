...
//! # Note on shared code
//!
//! This module duplicates post-processing logic from the `classify` crate.
//! Both are intentionally self-contained for now.
//! TODO: extract to a shared `imagenet-postprocess` crate alongside the
//! preprocess TODO (see roadmap.md "shared crates" milestone).
...
pub fn top_k_softmax(logits: &[f32], k: usize) -> Vec<(usize, f32)> {
    ...
    // Partial sort: bring the top-k elements to the front, then sort that slice.
